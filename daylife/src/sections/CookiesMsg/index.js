...
const CookiesMsg = () => {
    ...
    return (
        <div className='fixed bottom-0 z-100 bg-primary w-full shadow-cookies'>
            <div className={`block md:flex justify-between  max-w-[1900px] mx-auto  p-4 md:p-5 `}>
                ...
                <div className="flex justify-center flex-wrap gap-2 md:block">
                    ...
                    <Link aria-label="Page privacy" className={`group bg-primary overflow-hidden relative z-[1] shadow-2xl border-[1px] border-white text-white ${router.pathname.includes('privacy-policy') ? 'hidden' : 'flex'} items-center px-[20px] min-h-[50px] rounded-[50px] hover:bg-hover active:bg-active  min-w-fit !bg-transparent small !text-sm md:!text-base !justify-center md:justify-start`} href={`/privacy-policy/`}>{buttons[1].title}</Link>
                </div>
            </div>
        </div>
    )
